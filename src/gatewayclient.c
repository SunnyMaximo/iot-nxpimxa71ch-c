...
int subscribeToGatewayCommands(iotfclient  *client)
{
	...
	sprintf(logHdr,"%s:%d:%s",__FILE__,__LINE__,__func__);
        sprintf(logStr,"Calling MQTTClient_subscribe for subscribing to gateway commands");
        LOG(logHdr,logStr);
...
void gatewayMessageArrived(void *context, char *topicName, int topicLen, MQTTClient_message * message)
{
       sprintf(logHdr,"%s:%d:%s:",__FILE__,__LINE__,__func__);
       LOG(logHdr,"entry::");
